...
func helpKeyTokens() map[string]bool {
	...
	for _, section := range helpSections(keybind.DefaultSession(), keybind.DefaultList()) {
...
func TestHelpListRowsFollowTheKeyTable(t *testing.T) {
	...
	for _, section := range helpSections(keybind.DefaultSession(), list) {
		for _, row := range section.rows {
...
func TestHelpSearchNarrowsToMatchingRows(t *testing.T) {
	all := helpSections(keybind.DefaultSession(), keybind.DefaultList())
...
func TestHelpSearchIsCaseInsensitive(t *testing.T) {
	lower := helpRowCount(matchHelp(helpSections(keybind.DefaultSession(), keybind.DefaultList()), "revive"))
	upper := helpRowCount(matchHelp(helpSections(keybind.DefaultSession(), keybind.DefaultList()), "REVIVE"))
...
func TestHelpSearchOnASectionTitleKeepsItsRows(t *testing.T) {
	...
	for _, section := range matchHelp(helpSections(keybind.DefaultSession(), keybind.DefaultList()), "review") {
...
func TestHelpSessionRowsFollowTheKeyTable(t *testing.T) {
	rowFor := func(keys keybind.Table, key string) (string, bool) {
		for _, section := range helpSections(keys, keybind.DefaultList()) {
