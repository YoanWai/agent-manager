...
The id arrives one of two ways: tools with a `session_id_flag` launch under an id the manager mints, and tools that mint their own are read back by a `session_store` capturer (`codex`, `opencode`, `gemini`, `hermes`, `command-code`). The row shows the id once it is known. Without one, a tool with a `resume_picker_command` opens its own session picker in the pane on revive (`claude --resume`, `codex resume`, `cmd --resume`), so the user picks the conversation; tools without a picker fall back to `revive_command` (`claude --continue`), which resumes the working directory's most recent conversation, and the manager says so in the status line, since sessions sharing a directory would otherwise land on the wrong one. On a group row `v` revives every dead session under it, and `V` revives every dead session in view; both revive what they can and name the first failure rather than stopping.
