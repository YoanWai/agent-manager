...
When a session's status flips to `waiting` or `errored`, the manager fires one desktop notification with the session name, tool, and state — once per transition, never per poll — so you can look away from the list without missing an agent that needs you. On macOS, waiting, finished, and errored notifications use the Funk, Hero, and Basso sounds respectively. Inside Ghostty or cmux the ping is an OSC 777 escape to the drawing terminal, which turns it into a native notification attributed to the right window and workspace; because the escape rides the terminal connection, it reaches you even when the manager runs on a remote host over SSH. Elsewhere macOS posts via `osascript` and Linux via `notify-send`; with no desktop path at all — a headless box or WSL — the terminal bell is the fallback. Settings (`s`) has a `notifications` row that silences them (on by default) and a `notify on finish` row that adds `finished` transitions (off by default).
