...
At full width a session row is two lines and a group row is one, at any density: the layout has its own rhythm, and the list density setting shapes the split list. A session's first line carries the mark, name and badges, then the last prompt the manager delivered to the session (a quick prompt answer, a message from another agent, or the prompt it launched on), with `state · tool · age` against the right edge. The second quotes what the state makes worth reading: a `waiting` session shows the line it is blocked on, tinted in its state color; `working` and `finished` show the agent's last output line, read from above its input box; a `working` session that has not said anything yet animates a loader; the rest hold the line with a dash. The meters and messages condense to one line above the key legend: cpu, mem and net inline, plus a messages count that `M` opens.
